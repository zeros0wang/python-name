...
目标：掌握 Python 条件判断、循环控制和集合遍历，为后续处理 Agent 消息、工具列表和检索结果打基础。

运行练习：

```bash
.venv/bin/python src/day04/control_flow_practice.py
```

当天完成标准：

- 能使用 `if` / `elif` / `else` 完成多分支判断。
- 能使用 `for` 遍历 `list` 和 `dict`。
- 能使用 `range()` 控制循环次数。
- 能理解并使用 `break` 和 `continue`。
- 能用条件和循环筛选列表中的数据。
- 能把判断逻辑封装成函数。
